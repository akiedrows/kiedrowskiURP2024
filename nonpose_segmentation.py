...
blobs_labels, num_connected_comp = measure.label(
    binary_channel, background=0, return_num=True
)
...
watershed_labels = watershed(
    -distance, markers, mask=binary_channel, connectivity=2, watershed_line=False
)
...
index_components.save_ROIs(
    min_area, max_area, buffer_size, img_rescaled, basename, output_dir
)
