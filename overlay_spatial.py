...
img_rescaled = exposure.rescale_intensity(
    original_image, in_range="image", out_range=np.uint8
)
...
colored_image_rgba = np.concatenate(
    [colored_image, np.ones((colored_image.shape[0], colored_image.shape[1], 1)) * 0.5],
    axis=-1,
)
...
combined_image = ski.color.label2rgb(
    colored_image_rgba, image=img_rescaled_color, alpha=0.5
)
