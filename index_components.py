...
def properties_table(index_labels, img_rescaled, basename, output_dir):
    ...
    props_df.to_csv(
        os.path.join(output_dir, f"{basename}_propertiestable.csv"), index=False
    )
        ...
        if min_area < area < max_area:
            ...
            if np.any(object_img):
                ...
                if object_img.dtype != np.uint16:
                    object_img = (object_img / np.max(object_img) * 65535).astype(
                        np.uint16
                    )
...
def grayco_features(isolated_img_gray):
    ...
    for (x, y), area, label in zip(centroids, areas, labels):
        if 500 < area < 30000:
            ...
            isolated_img = np.zeros_like(
                blobs_img[y_min:y_max, x_min:x_max], dtype=np.uint8
            )
            ...
            if (
                np.max(isolated_with_values) > 0
            ):  # if the isolated image has non-zero values
                isolated_with_values = (
                    isolated_with_values / np.max(isolated_with_values) * 65535
                ).astype(np.uint16)
                isolated_object_filename = f"isolated_object{label:04d}.png"
                isolated_object_filepath = os.path.join(
                    isolated_objects_dir, isolated_object_filename
                )
                ...
                contrast, homogeneity, ASM, correlation = grayco_features(
                    isolated_img_gray
                )
                grayco_data.append(
                    {
    ...
    for (x, y), area, label in zip(centroids, areas, labels):
        if 500 < area < 30000:
            ...
            if (
                ...
                print(
                    f"Object area: {area}, half_box_size: {half_box_size}, coordinates: ({x_min}, {x_max}, {y_min}, {y_max})"
                )
...
def isolate_ROIs(min_area, max_area, buffer_size, image, masks, basename, output_dir):
    ...
    for (x, y), area, label in zip(centroids, areas, labels):
        if min_area < area < max_area:
            ...
            isolated_img = np.zeros_like(
                blobs_img[y_min:y_max, x_min:x_max], dtype=np.uint16
            )
            ...
            isolated_with_values_scaled = ski.exposure.rescale_intensity(
                isolated_with_values, in_range="image", out_range=np.uint8
            )
            contrast, homogeneity, ASM, correlation = grayco_features(
                isolated_with_values_scaled
            )
            grayco_data.append(
                {
                    "label": label,
